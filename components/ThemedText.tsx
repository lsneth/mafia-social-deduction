...
export function ThemedText({ className, type = 'default', underline = false, ...rest }: ThemedTextProps): JSX.Element {
  return (
    <Text
      ...
      className={`text-mafiaWhite ${type === 'title' ? 'pt-2 text-7xl' : type === 'title-sm' ? 'pt-1 text-5xl' : type === 'subtitle' ? 'text-3xl' : 'text-lg'} ${underline ? 'underline' : ''} text-center ${className}`}
      {...rest}
    />
  )
}
